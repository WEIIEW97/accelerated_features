...
namespace xfeat {
  Detector::Detector(const std::string& weights_path, int top_k,
                     float detection_threshold, bool use_cuda)
      : top_k(top_k), detection_threshold(detection_threshold) {
    // load the model
    ...
    device_type =
        (use_cuda && torch::cuda::is_available()) ? torch::kCUDA : torch::kCPU;
  ...
  void Detector::detect_and_compute(
      torch::Tensor& x,
    ...
    float rh1, rw1;
    std::tie(x, rh1, rw1) = preprocess_tensor(x);
    ...
    torch::Tensor K1h = get_kpts_heatmap(K1);
    torch::Tensor mkpts = NMS(K1h, detection_threshold, 5);

    // compute reliability scores
    auto scores = (nearest->forward(K1h, mkpts, _H1, _W1) *
                   bilinear->forward(H1, mkpts, _H1, _W1))
                      .squeeze(-1);
    ...
    scores = scores.gather(-1, idxs).index(
        {torch::indexing::Slice(), torch::indexing::Slice(0, top_k)});
    ...
    feats = torch::nn::functional::normalize(
        feats, torch::nn::functional::NormalizeFuncOptions().dim(-1));

    // correct kpt scale
    torch::Tensor scaling_factors =
        torch::tensor({rw1, rh1}, mkpts.options()).view({1, 1, -1});
  ...
  void Detector::match(torch::Tensor& feats1, torch::Tensor& feats2,
  ...
  void Detector::match_xfeat(cv::Mat& img1, cv::Mat& img2, cv::Mat& mkpts_0,
                             cv::Mat& mkpts_1) {
    torch::Tensor tensor_img1 = parse_input(img1);
    torch::Tensor tensor_img2 = parse_input(img2);

    std::unordered_map<std::string, at::Tensor> out1, out2;
    detect_and_compute(tensor_img1, out1); // no batches
    detect_and_compute(tensor_img2, out2);
    ...
    mkpts_0 = tensor2mat(mkpts_0_tensor);
    mkpts_1 = tensor2mat(mkpts_1_tensor);
  }

  torch::Tensor Detector::parse_input(cv::Mat& img) {
    // if the image is grayscale
    if (img.channels() == 1) {
      torch::Tensor tensor =
          torch::from_blob(img.data, {1, img.rows, img.cols, 1}, torch::kByte);
    ...
    throw std::invalid_argument(
        "Unsupported number of channels in the input image.");
  }

  std::tuple<torch::Tensor, double, double>
  Detector::preprocess_tensor(torch::Tensor& x) {
  ...
  torch::Tensor Detector::get_kpts_heatmap(torch::Tensor& kpts,
                                           float softmax_temp) {
    torch::Tensor scores = torch::nn::functional::softmax(
        kpts * softmax_temp, torch::nn::functional::SoftmaxFuncOptions(1));
    scores =
        scores.index({torch::indexing::Slice(), torch::indexing::Slice(0, 64),
  ...
  torch::Tensor Detector::NMS(torch::Tensor& x, float threshold,
                              int kernel_size) {
    int B = x.size(0);
    int H = x.size(2);
    int W = x.size(3);
    ...
    auto local_max = torch::nn::functional::max_pool2d(
        x, torch::nn::functional::MaxPool2dFuncOptions(kernel_size)
               .stride(1)
               .padding(pad));
    ...
    for (int b = 0; b < pos.size(0); ++b) {
      ...
      k = k.index({torch::indexing::Ellipsis,
    ...
    torch::Tensor pos_tensor = torch::zeros(
        {B, pad_val, 2},
  ...
  cv::Mat Detector::tensor2mat(const torch::Tensor& tensor) {
    ...
    std::memcpy(mat.data, cpu_tensor.data_ptr<float>(),
                cpu_tensor.numel() * sizeof(float));
    return mat;
  }

  std::string Detector::get_weights_path(std::string weights) {
